...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = Join-Path $repoRoot "artifacts\RF-Comp.rfplugin"
}
...
try {
    ...
    cargo "+$Toolchain" run --locked --release -p rf-comp-lab -- metadata
    if ($LASTEXITCODE -ne 0) { throw "RF-Comp metadata generation failed" }
    ...
    cargo "+$Toolchain" build --locked --release -p rackforge-rf-comp --target wasm32-unknown-unknown
    if ($LASTEXITCODE -ne 0) { throw "RF-Comp WebAssembly build failed" }

    cargo "+$Toolchain" test --locked --release --workspace
    if ($LASTEXITCODE -ne 0) { throw "RF-Comp tests failed" }

    $outputParent = Split-Path -Parent $Output
    New-Item -ItemType Directory -Path $outputParent -Force | Out-Null
    $stage = Join-Path ([System.IO.Path]::GetTempPath()) ("rf-comp-package-" + [guid]::NewGuid().ToString("N"))
    ...
    try {
        ...
        $component = Join-Path $repoRoot "target\wasm32-unknown-unknown\release\rackforge_rf_comp.wasm"
