#!/usr/bin/env bash
# Builds RF-Comp and packs it with the RackForge packager, which lives in
# a sibling checkout (or wherever RACKFORGE_ROOT points).
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
output="${1:-$repo_root/artifacts/RF-Comp.rfplugin}"
...
cd "$repo_root"
cargo run --locked --release -p rf-comp-lab -- metadata
...
cargo build --locked --release -p rackforge-rf-comp --target wasm32-unknown-unknown
...
cargo run --manifest-path "$rackforge_root/Cargo.toml" --locked -p rackforge-store -- \
  pack-wasm "$stage" \
  "$repo_root/target/wasm32-unknown-unknown/release/rackforge_rf_comp.wasm" \
  "$output"
echo "Packed $output"
