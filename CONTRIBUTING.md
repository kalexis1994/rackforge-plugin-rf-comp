# Contributing to RF-Comp

## The rule that matters most

A constant that shapes the behaviour should be traceable to something outside
somebody's taste: a time, a window, a floor, a measurement. When that is not
yet true, say so where the code is.
...
```bash
cargo run -p rf-comp-lab -- metadata
cargo run -p rf-comp-lab -- metadata --check    # what CI runs
...
1. Add it to `rf-comp-contract` (`lib.rs`, `index.rs`) at the end and bump
...
3. Tests: what the compressor is supposed to *do*, not what the code currently
   returns. "Twelve over at four to one takes nine off", "all dry is the
   input itself", "the attack reaches its time constant".
4. `cargo run -p rf-comp-lab -- metadata`.
